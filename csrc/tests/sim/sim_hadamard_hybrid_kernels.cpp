 ...
 * CHUNKING
 *
 * UB is the binding constraint: the pipeline holds fp32 input, an fp16 cast,
 * the fp32 product and an fp32 ping-pong buffer, so 14 bytes per element. The
 * host sizes a chunk and the kernel loops over chunks, each a self-contained
 * stage -> Mmad -> Fixpipe -> residual -> writeback. The host guarantees
...
// Cross-core flag ids, matching the decode's convention: the AIV signals
// "operands are in L1", the AIC signals "the product is in UB".
...
public:
                                ...
                                uint32_t vectorsPerChunk, uint32_t variant, float invSqrtDim)
    {
        ...
        pipe_->InitBuffer(aHi1_, paddedElems_ * sizeof(half));
        pipe_->InitBuffer(aLo1_, paddedElems_ * sizeof(half));
        ...
        pipe_->InitBuffer(prodBuf_, chunkElems_ * sizeof(float));
        pipe_->InitBuffer(tmpBuf_, chunkElems_ * sizeof(float));
    }
    ...
    __aicore__ inline void Process()
    {
        ...
        for (uint32_t chunk = 0; chunk < numChunks_; ++chunk) {
            ...
            if ASCEND_IS_AIV {
                StageOperands(baseVector);
                // Both subcores set the flag. One AIC wait is released by the
                // pair, so a set from only one of them hangs the Cube; see the
                // addendum in turboquant_mm_kernels.cpp's SignalOperandsReady.
...
private:
     ...
     */
    __aicore__ inline void StageOperands(uint32_t baseVector)
    {
        ...
        SyncVectorToMte3();
        AscendC::DataCopy(aHi1_.Get<half>(), cast, paddedElems_);
        ...
        if ((variant_ & kVariantHiLo) != 0) {
            ...
            AscendC::Cast(cast, in, AscendC::RoundMode::CAST_RINT, paddedElems_);
            SyncVectorToMte3();
            AscendC::DataCopy(aLo1_.Get<half>(), cast, paddedElems_);
        }
    }
    ...
    // hi/lo split is on, accumulating into the same L0C.
    __aicore__ inline void RunCube(const AscendC::LocalTensor<float> &dstUb)
    {
        const bool hiLo = (variant_ & kVariantHiLo) != 0;
        LoadA(aHi1_.Get<half>(), a2Hi_.Get<half>());
        if (hiLo) {
            LoadA(aLo1_.Get<half>(), a2Lo_.Get<half>());
        }
        LoadB();
        ...
        AscendC::Fixpipe<float, float, kFixpipeToUb>(dstUb, acc, fp);
     ...
     */
    __aicore__ inline void Residual(const AscendC::LocalTensor<float> &prod, uint32_t baseVector)
    {
        ...
        AscendC::LocalTensor<float> tmp = tmpBuf_.Get<float>();
        AscendC::LocalTensor<float> src = prod;
        ...
        // MTE3 -> V, because the next chunk's Fixpipe and residual write these
        // buffers while this DMA may still be reading one of them.
        SyncEvent<AscendC::HardEvent::MTE3_V>();
    }

    AscendC::TPipe *pipe_;
    AscendC::TBuf<AscendC::TPosition::A1> aHi1_;
    AscendC::TBuf<AscendC::TPosition::A1> aLo1_;
